	...
	class m_katu extends CI_Model{
		...
		function get_kegiatan_pegawai_caraka($peg){
			$tgl = date("Y-m-d");
			$sql = $this->db->query("select id_kegiatan,waktu,(select nama_kegiatan from kegiatan_kebersihan where id_nama=k.id_nama) as nk from detail_kegiatan_kebersihan k where (select id_pegawai from presensi where id_presensi=k.id_presensi)='$peg' and substr(waktu,1,10)='$tgl'");
		   return $sql;
		...
		function insert_form_ketidakhadiran(){
			...
			$query= $this->db->query("insert into presensi  
				values ('','$tgl','$status_presensi','$alasan','$id_pegawai','$jdw')");
		...
		function insert_penilaian(){
			...
			$query= $this->db->query("insert into penilaian (periode,absensi,kegiatan,keramahan,id_pegawai)
				values ('$periode','$absensi','$kegiatan','$keramahan','$pegawai')");
		...
		function get_rekap_pengajuan(){
			$query=$this->db->query("SELECT DP.progress,DP.iddetail,P.NAMA,PB.ID_PENGAJUAN, PB.TGL_PENGAJUAN ,B.NAMA_BARANG, B.MERK_BARANG, DP.IDDETAIL, DP.JUMLAH_PERMINTAAN FROM PEGAWAI P JOIN PENGAJUAN_BARANG PB ON (P.ID_PEGAWAI=PB.ID_PEGAWAI) JOIN DETAIL_PENGAJUAN DP ON (PB.ID_PENGAJUAN=DP.ID_PENGAJUAN) JOiN BARANG B ON (DP.KODE_BARANG=B.KODE_BARANG) where progress =''");
			
			return $query;
		}
		function get_rekap_pengajuan_yang_diajukan(){
			$query=$this->db->query("SELECT DP.TGL_PROGRES,DP.TGL_TERSEDIA,DP.TGL_DIAMBIL,DP.barang_diambil,DP.barang_tersedia,DP.progress,DP.iddetail,P.NAMA,PB.ID_PENGAJUAN, PB.TGL_PENGAJUAN ,B.NAMA_BARANG, B.MERK_BARANG, DP.IDDETAIL, DP.JUMLAH_PERMINTAAN FROM PEGAWAI P JOIN PENGAJUAN_BARANG PB ON (P.ID_PEGAWAI=PB.ID_PEGAWAI) JOIN DETAIL_PENGAJUAN DP ON (PB.ID_PENGAJUAN=DP.ID_PENGAJUAN) JOiN BARANG B ON (DP.KODE_BARANG=B.KODE_BARANG) where progress ='Progress'");
		...
		function getProgres($id){
			...
			$this->db->set('tgl_progres',date('d/m/Y'));
		...
		function setKetersediaan($id){
			$this->db->set('barang_tersedia','Barang Tersedia');
			$this->db->set('tgl_tersedia',date('d/m/Y'));
			$this->db->where('iddetail',$id);
			$this->db->update('detail_pengajuan');
		}
function upd_pengambilan($id){
			$this->db->set('barang_diambil','Barang diambil');
			$this->db->set('tgl_diambil',date('d/m/Y'));
			$this->db->where('iddetail',$id);
			$this->db->update('detail_pengajuan');
		}
